...
export default function PdfToWordPage() {
  ...
  const handleConvert = useCallback(async () => {
    ...
    try {
      ...
      // 1) pdfjs-dist 로 텍스트 추출을 시도 (클라이언트 사이드 간이 변환)
      //    - 레이아웃 보존은 어렵지만, Word에서 열 수 있는 유효한 문서를 생성
      let extractedText = '';
      try {
        // 동적 로딩: 빌드에 미설치 시에도 앱이 깨지지 않도록 처리
        ...
        // pdfjs 미설치/로딩 실패 시 텍스트 추출 없이 진행 (폴백 경로로 이동)
        console.warn('PDF 텍스트 추출 실패, 폴백 경로로 진행합니다.', pdfErr);
      }

      // 2) docx 라이브러리로 유효한 DOCX 생성 시도
      try {
        ...
        const docxBuffer: ArrayBuffer = await Packer.toArrayBuffer(doc);
        const convertedBlob = new Blob([docxBuffer], {
      ...
      const fallbackBlob = new Blob([htmlDoc], { type: 'application/msword' });
      ...
      <main className='mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8'>
        ...
        <nav className='mb-6 text-sm text-gray-600'>
          <ol className='flex items-center space-x-2'>
            ...
            <li className='text-gray-900 font-medium'>PDF 워드 변환</li>
          </ol>
        </nav>
              ...
              <div className='flex items-center justify-between'>
                <div>
                  ...
                  <p className='mt-1 text-xs text-gray-600'>
                    스캔된 PDF의 경우 OCR을 사용하여 텍스트를 인식할 수 있습니다
                  </p>
                </div>
        ...
        <div className='mb-12 rounded-2xl border border-gray-200 bg-white p-8'>
          ...
          <p className='text-center text-gray-700 mb-8 max-w-3xl mx-auto'>
            ...
            Microsoft Word 문서로 변환해 보세요. 무료이고 바로 다운로드할 수
            있어요.
          </p>
        </div>
        ...
        <div className='mb-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3'>
          ...
          <div className='rounded-xl border border-gray-200 bg-white p-6'>
            ...
            <p className='text-sm text-gray-600'>
              PDF를 DOCX로 변환하는 것이 복잡할 필요는 없습니다. Solid Documents
        ...
        <div className='mb-12 rounded-2xl border border-gray-200 bg-white p-8'>
          ...
          <div className='space-y-6'>
            ...
            <div className='rounded-lg border border-gray-200 bg-gray-50 p-6'>
              <h3 className='mb-2 text-lg font-semibold text-gray-900'>
                다른 Microsoft Office 형식으로도 변환할 수 있나요?
              </h3>
              <p className='text-sm text-gray-700'>
            ...
            <div className='rounded-lg border border-gray-200 bg-gray-50 p-6'>
              ...
              <p className='text-sm text-gray-700'>
                물론입니다! Solid documents와 Abby의 기술 덕분에 글꼴, 레이아웃,
                형식이 원본 그대로 유지됩니다. 변환 후 문서도 원본과 똑같이
                보입니다.
              </p>
            </div>
            <div className='rounded-lg border border-gray-200 bg-gray-50 p-6'>
